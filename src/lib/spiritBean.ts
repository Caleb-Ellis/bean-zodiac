import { type BeanId, type FlavourId, type FormId, type QualityId, type ZodiacId } from "./zodiac";
...
export const SPIRIT_FLAVOUR_RING: FlavourId[] = ["bitter", "sour", "spicy", "sweet", "umami"];
...
export function getRingAdjustment(rawScore: number, qualityId: QualityId): RingAdjustment {
...
export function computeSpiritBeanScores(
  ...
  const [claimedFlavourId, claimedFormId, claimedBeanId] = claimedSlug.split("-") as [
    FlavourId,
    FormId,
    BeanId,
  ];

  const flavourScores = Object.fromEntries(SPIRIT_FLAVOUR_RING.map((id) => [id, 10]));
  ...
  const flavourValues = SPIRIT_FLAVOUR_RING.map((id) => Math.max(0, flavourScores[id]));
...
export function getSpiritDiff(scores: SpiritBeanScores): number {
  return (
    ...
    (Math.max(...scores.formValues) - scores.formValues[scores.claimedFormIdx]) +
...
export function buildBeanstalkNodes(claimedSlug: ZodiacId): BeanstalkNode[] {
  const history = useStore.getState().fortuneHistory.sort((a, b) => (a.date < b.date ? -1 : 1));
