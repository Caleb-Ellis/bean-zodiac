...
function attrBadge(
  ...
  if (attr === "bean") return <BeanBadge small id={meta.beanId} name={bean.name} />;
  if (attr === "flavour") return <FlavourBadge small id={meta.flavourId} name={flavour.name} />;
  ...
  return (
    <div className="bg-zinc-900/80 border-2 border-zinc-800 rounded-xl px-5 py-4 flex items-center gap-4">
      ...
      <div className="flex flex-col gap-0.5">
        <span className="font-semibold text-white text-sm text-left">{compat.label}</span>
        <span className="flex items-center gap-1 text-xs my-1.5 flex-wrap">
          {badgeA}
          <span className="text-zinc-600">×</span>
          {badgeB}
        </span>
        <span className="text-zinc-400 text-sm text-left">{compat.description}</span>
      </div>
    </div>
  );
}
          ...
          {revealedCount >= 4 && specialDetail && (
            <div className="animate-fade-up">
              <DimensionRow
                ...
                badgeA={attrBadge(specialDetail.attrA, metaA, beanA, flavourA, formA)}
                badgeB={attrBadge(specialDetail.attrB, metaB, beanB, flavourB, formB)}
              />
            </div>
          )}
        </div>
