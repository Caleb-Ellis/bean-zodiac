...
import { SPIRIT_BEAN_RING, SPIRIT_FLAVOUR_RING, SPIRIT_FORM_RING } from "../../lib/spiritBean";
...
export function FlavourRadar({
  ...
  const labelHrefs = showLinks ? SPIRIT_FLAVOUR_RING.map((id) => `/flavours/${id}`) : undefined;
  return (
    <SpiritBeanRadar
      labels={labels}
...
export function FormRadar({ data, claimedId, values, highlightIndex, showLinks }: FormRadarProps) {
  ...
  const labelHrefs = showLinks ? SPIRIT_FORM_RING.map((id) => `/forms/${id}`) : undefined;
  return (
    <SpiritBeanRadar
      labels={labels}
      labelColors={labelColors}
...
export function BeanRadar({ data, claimedId, values, highlightIndex, showLinks }: BeanRadarProps) {
  ...
  const labelHrefs = showLinks ? SPIRIT_BEAN_RING.map((id) => `/beans/${id}`) : undefined;
  return (
    <SpiritBeanRadar
      labels={labels}
      labelColors={labelColors}
