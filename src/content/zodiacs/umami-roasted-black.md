...
fortuneHigh2: Follow the small wrong note you caught and everyone else let pass.
fortuneMid: Walk away from the thing you're stuck on. Out on the road, the load-bearing piece surfaces on its own.
fortuneMid2: You read fast. The slow read is the truer one — give it the hour it needs.
fortuneLow: Sometimes "I'm fine" is just the truth, and the kind thing is to take it.
fortuneLow2: Not every seam is yours to mend.
...
fortuneLeast2: A plan quietly stopped working a while back. Pick up the next one.
...
The shadow is that a read once taken is hard for them to release. The friend they saw clearly at twenty-three is still that person in their head a decade later, even as everyone else has updated the file.
