---
slug: spicy-fermented-green
bean: green
flavour: spicy
form: fermented
trait: unconventional
dish: Green bean doenjang muchim - seasoned green beans with fermented soybean paste and gochugaru.
quote: Here's to the crazy beans, the misfits, the rebels, the troublemakers.
creature: duck-billed platypus
friendlyBeans: [kidney, pinto]
antiBeans: [fava, butter]
friendlyForm: roasted
antiForm: boiled
seasonalFortune: Crocks held in the dark this season yield what no kitchen expected. The gochugaru deepens; the bean keeps its snap. Households reach for jars they cannot quite name.
fortuneMost: A jar the household can't quite name, and the right answer reached by a route that's on no map and can't be retraced even by you.
fortuneMost2: The recipe written down only after the third batch, in your own shorthand, margins left untranslated.
fortuneHigh: Asked how you got here, take the scenic way through every dead end.
fortuneHigh2: Cupboard half-empty, shops shut, and you improvise something stranger and better than the recipe would have made.
fortuneMid: A painting hung where the light is wrong, and the room finally makes sense.
fortuneMid2: One odd substitution and a note for next time gets you most of the way there.
fortuneLow: A half-formed bridge offered now beats a polished one offered after a long silence.
fortuneLow2: Return the dish washed and on time, the thanks already on its way.
fortuneLeast: Sketch your way back to where you split off, so someone else can follow.
fortuneLeast2: The same bus, the same chair, the bowl you always eat from, loved exactly as they are.
facetMostTitle: The Untranslated Margin
facetMost: Take the zigzag of alleys and back streets only you would ever string together to cross the city. You'll arrive, and you'd never be able to draw the route for anyone after.
facetHighTitle: One Good Knife
facetHigh: The instructions for the flat-pack are right there in the box, clear as anything. You shove them aside and build it by feel, the way the shape of the thing tells you to.
facetMidTitle: Doenjang, Then a Swerve
facetMid: Everyone fills the new notebook front to back. You start in the middle, run notes in both directions, and number the pages your own way.
facetLowTitle: The Lid Taped on
facetLow: The clever sideways version of this errand isn't calling to you the way it usually would. The plain method is simpler and you reach for it without a second thought.
facetLeastTitle: The Jar in Its Place
facetLeast: Teaching someone a skill you'd normally do by feel, you set the improvising aside entirely. You break it into plain numbered steps and walk them through it the standard, teachable way.
question: A simple recipe, the staples missing, the shops closed for the night. You...
answerMost: improvise something stranger and better from whatever the cupboard actually contains
answerHigh: swap two ingredients for cousins and see what happens
answerMid: get most of the way there with one substitution and a note for next time
answerLow: order in and try the recipe properly tomorrow
answerLeast: have toast; the recipe will keep
rorschachMost: a crescent moon beside an uncharted burst
rorschachHigh: a feather drifting past a thistle head
rorschachMid: a banana beside a chestnut in its husk
rorschachLow: a sickle resting next to a pinecone
rorschachLeast: a ruler laid beside a stamped seal
---

Kimchi Green Beans are the Bean Zodiac's most unconventional — the ones who arrive at the right answer by a route that doesn't appear on any map and can't be retraced even by them. There's a notebook somewhere, half in shorthand, half in arrows, and a finished thing on the counter that everyone wants the recipe for.

When asked, they hand over the working scribble and seem genuinely puzzled that it's not enough. The route was the point; cleaning it up into steps loses the thing that made it work, but leaving it raw loses the people who wanted to follow.
