---
slug: spicy-dried-cannellini
bean: cannellini
flavour: spicy
form: dried
trait: unerring
dish: Pasta e fagioli - dried cannellini and pasta soup with peperoncino.
quote: I came, I saw, I beaned.
seasonalFortune: Dried beans hold their shape through long heat; peperoncino leaves a mark that outlasts the bowl. So with this one — measured, and lingering.
fortuneMost: Sign once. A second pass is doubt in handwriting.
fortuneHigh: One mark, hard. The underline is the apology.
fortuneMid: Read it aloud before you send. The page drifted from your head.
fortuneLow: Ship the draft rough. Polish strands it.
fortuneLeast: Nod and move. Three asks don't need a full sentence.
facetMostTitle: Every Seed Counted
facetMost: You finish a piece of work only when the last comma has earned its place.
facetHighTitle: The Overnight Soak
facetHigh: When you owe an apology, you send the exact one — naming the thing, not the weather around it.
facetMidTitle: One Steady Note
facetMid: You cook the same six dinners and notice when the salt is off by a pinch.
facetLowTitle: Where the Spoon Rested
facetLow: You let books stack where they fall and find what you need by the shape of the room.
facetLeastTitle: Scattered Heat
facetLeast: Tabs multiply; replies arrive whenever; nothing filed, nothing missed.
question: A colleague hands you a draft and asks for "any quick thoughts." You see four real problems. You...
answerMost: mark all four, precisely, and send it back with a numbered list
answerHigh: name the two that matter and let the rest stand
answerMid: flag one big thing and say the rest is solid
answerLow: say it reads well and make a mental note of the issues
answerLeast: thumbs-up and move on; not your draft, not your problem
rorschachMost: two beaks meeting exactly across a hair's gap
rorschachHigh: a wolf's head squared off against its mirror
rorschachMid: two animal heads facing across a seam
rorschachLow: two heads turned away, droplets scattering between
rorschachLeast: two faces drifting apart, edges flaking off
---

Sichuan Cannellini Beans are the Bean Zodiac's most unerring — the friend whose one-line note on your draft is the note you should have written yourself. They don't pad and they don't soften. When they say the third paragraph isn't earning its place, the third paragraph isn't earning its place, and you know it before you've finished reading their message.

The cost is that an offhand comment from them lands like a verdict, and they sometimes forget this. A passing "that jacket's a choice" said on a Tuesday is still being thought about on Friday.
