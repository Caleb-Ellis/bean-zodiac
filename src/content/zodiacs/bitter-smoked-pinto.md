...
fortuneHigh: One small detail is carrying more than the whole structure around it. Pare down to it, let it take the weight, and trust the depth still shows through.
fortuneHigh2: Something good waits at the end of the one load-bearing thread. You can name the line between any two — follow the quiet one all the way out.
fortuneMid: Pick the single thread you'd hand a stranger and let the other six stay home. One line followed to the end beats six left half-drawn.
fortuneMid2: The next layer is for you, not for them. Give the one that lands and keep the rest of the build to yourself this time.
fortuneLow: The short answer is the right one. The extra you keep layering on is for you, not the person waiting — set it down before it buries the point, even if leaving it out feels like losing the truth.
fortuneLow2: You'll want to redraw the whole route rather than let someone walk it themselves. Hand it over plain. The version they can actually follow is the one that helps.
...
The catch is that almost no one stays. They mistake the complexity for the point, and so the email runs to a third paragraph written for themselves rather than the reader, and the story that could be told to an eight-year-old gets the full architecture instead. They are the only one holding the map, and they keep redrawing it rather than letting a single thread be followed all the way out.
