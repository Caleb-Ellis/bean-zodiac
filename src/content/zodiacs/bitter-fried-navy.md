---
# TODO: tidy facets, redo rorschachs
# SHOULD BE: trait severe, excess punitive, inverse lenient
slug: bitter-fried-navy
lastUpdated: 
bean: navy
flavour: bitter
form: fried
trait: incisive
excess: cutting
inverse: woolly
quote: You see the bean, but you do not observe it.
creature: goshawk
dish: Navy beans pan-scorched with rosemary and burnt garlic oil.
friendlyBeans: [cannellini, edamame]
friendlyForm: dried
antiTriple: sweet-fermented-adzuki
antiBeans: [fava, butter]
antiForm: smoked
seasonalFortune: "A season of burnt garlic and clean cuts. The pan removes the argument and leaves the point."
fortuneMost: Cut to the point sharply enough and the point stops arriving.
fortuneHigh: A goshawk goes through the hedge rather than round it.
fortuneMid: The clean summary saves an hour.
fortuneLow: A ramble does no harm.
fortuneLeast: The woolly version has things in it the sharp one loses.
facetMostTitle: Kill Bean
facetMost: Forty minutes of costing and everybody has talked around the same number. You say the job loses money at that rate. The man who set the rate is in the room.
facetHighTitle: Occam's Bean
facetHigh: A violin has been to two people and neither could say what was wrong with it. You have it under the lamp a minute and tell her the bass bar has gone.
facetMidTitle: Sharp Beans
facetMid: A group has spent an hour on why the takings are down and has named four things that might be doing it, and you ask which of the four they would actually change.
facetLowTitle: Woolly Beanly
facetLow: The report is nine pages of hedging and the answer is on page seven if you know where to look, and there are eleven people who will act on the summary rather than the page. You let it go through with a comment about the formatting.
facetLeastTitle: Cloudy with a Chance of Beans
facetLeast: Somebody wants to know what went wrong on the job. You say a number of factors came together. Everybody nods.
question: The argument about the rota has run an hour and everyone is circling the same thing without naming it. You...
answerMost: name it in a sentence, with the man it's about sitting there
answerHigh: name it in one sentence and let the room stop
answerMid: put it as a question and let somebody else land it
answerLow: sum up where everyone agrees
answerLeast: allow there are good points on all sides
rorschachMost: a fistful of nettles
rorschachHigh: a cracked-open geode
rorschachMid: a split log
rorschachLow: a bobble hat
rorschachLeast: a ball of wool
---

Scorched Navy Beans are the Bean Zodiac's most incisive. One sentence and the argument that ran an hour is over, because they named the thing everyone was circling. The sentence sometimes takes skin with it. Vague, generous summaries of a problem make them visibly restless.
