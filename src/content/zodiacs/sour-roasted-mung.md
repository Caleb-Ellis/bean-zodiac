---
slug: sour-roasted-mung
bean: mung
flavour: sour
form: roasted
trait: sensitive
dish: Bhuna moong chaat - dry-roasted mung beans with tamarind chutney, red onion, and chaat masala
quote: Nothing is so strong as bean-ness.
creature: roe fawn
friendlyBeans: [butter, chickpea]
antiBeans: [edamame, kidney]
friendlyForm: boiled
antiForm: fried
seasonalFortune: A season when tamarind reaches further than expected. The mung's roasted skin catches every change in the room's air, and the bean keeps a record the cook does not.
fortuneMost: This turn the roasted skin catches every shift in the air and you read it true.
fortuneMost2: You walk in already knowing who left angry an hour ago.
fortuneHigh: Answer the one underneath it out loud — that was always what was being asked.
fortuneHigh2: That thin vowel was the whole call; clear what you can and go to them.
fortuneMid: You're the only one who caught the off-note.
fortuneMid2: That's the real one — reply to that.
fortuneLow: You can catch a thing and still leave it be.
fortuneLow2: You don't have to triage every flicker.
fortuneLeast: The phone left in another room while you sleep.
fortuneLeast2: Not every wince was sent to you to carry.
facetMostTitle: The Pan It Cooled in
facetMost: The cashier says they're fine, but the change lands a half-second too hard in your palm. You catch the wobble under it and stay, gently, to ask again.
facetHighTitle: Chaat Masala, Spice by Spice
facetHigh: Someone keeps asking the same sideways question, never the real one. You hear the question underneath and answer that one out loud instead.
facetMidTitle: Onion First, Tamarind After
facetMid: In a noisy group, you're the only one who catches the off-note in how a friend laughed. You name it gently, low, because no one else is going to.
facetLowTitle: The Tamarind, Swallowed
facetLow: In a crowded waiting room you pick up a stranger's private grief, plain as a draft under a door. You leave it be — it was never handed to you to hold.
facetLeastTitle: One Good Plate
facetLeast: A flicker of something crosses a face across the bus aisle. You let it pass unread — most flickers were never signals, and the real ones come back.
question: A friend's voice goes thin on one vowel during a quick catch-up call. You...
answerMost: cancel your evening and tell them you're coming over
answerHigh: stop the small talk and ask what's actually going on
answerMid: file it, finish the call warmly, and check in by text tonight
answerLow: note it and wait to see if they bring it up next time
answerLeast: carry on, assume it was nothing, and not think of it again
rorschachMost: an ear turned toward a faint speck of sound
rorschachHigh: a snail uncurling its shell toward a crumb
rorschachMid: a gnarled stump beside an empty bird's nest
rorschachLow: a sleeping bear ignoring the moth at its foot
rorschachLeast: a shuttered window facing a dropped seed
---

Chimichurri Mung Beans are the Bean Zodiac's most sensitive — they walk into a room and know who left it angry an hour ago. The friend's voice thins on one vowel and the evening gets quietly cleared; the message arrives with a second sentence the writer almost deleted, and that's the sentence that gets answered.

The shadow is that not every signal was sent on purpose. They pick up a flicker that wasn't meant to be caught, and end up holding something private that the other person hadn't decided to share yet.
