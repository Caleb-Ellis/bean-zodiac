...
// authored trait (friendly pole) and inverse (anti pole). The model is
// symmetric: each zodiac has a friendly pole (its own slug + friendlyBeans +
// friendlyForm) and an anti pole (antiTriple + antiBeans + antiForm). Never the
// zodiac's own bean/form/flavour; the anti-triple's bean/form are also barred
// from antiBeans/antiForm, so each pole covers 3 distinct beans and 2 distinct
// forms; and the two poles never share a bean or a form, since a tag on both
// poles moves the same way whatever the tier and so carries no signal.
...
function validateSpiritTags(id, data, own) {
  ...
  // ...and the two poles must not overlap each other.
  const friendlyPoleBeans = new Set([own.bean, ...data.friendlyBeans]);
  for (const tag of [tBean, ...data.antiBeans]) {
    if (friendlyPoleBeans.has(tag)) {
      throw new Error(`${id}: bean "${tag}" is on both the friendly and the anti pole`);
    }
  }
  const friendlyPoleForms = new Set([own.form, data.friendlyForm]);
  for (const tag of [tForm, data.antiForm]) {
    if (friendlyPoleForms.has(tag)) {
      throw new Error(`${id}: form "${tag}" is on both the friendly and the anti pole`);
    }
  }
}
